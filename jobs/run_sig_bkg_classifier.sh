...
# -- Editable entries --
LOAD_LAST=false
CONFIG=DNN_AN_1e-3_e20drop75_minDelta1em5_SPANet_inclusive_b_region_bratioAll_postEE_ArctanhDeltaProb_not_spanet_jet5_less_add_jet1pt_vars
i=100
...
if [[ "$LOAD_LAST" == true && ${#matches[@]} -gt 0 ]]; then
	...
	if [[ $LOAD_LAST == true ]]; then
		ml_train -o "$RUN_DIR" \
				 ...
				 --gpus 0 -n 2 -c "$CONFIG_FILE" -s "$SEED" --load-model $MODEL_PATH --comet-token $API_KEY --comet-name $API_UNAME --comet-tags "${API_TAGS[@]}" --overwrite
	else
		ml_train -o "$RUN_DIR" \
				 --eval --onnx --roc --histos --history\
			     --gpus 0 -n 2 -c "$CONFIG_FILE" -s "$SEED" --overwrite --comet-token $API_KEY --comet-name $API_UNAME --comet-tags "${API_TAGS[@]}" --overwrite
	fi
else
	if $LOAD_LAST; then
		ml_train -o "$RUN_DIR" \
				 --eval --onnx --roc --histos --history \
				 --gpus 0 -n 2 -c "$CONFIG_FILE" -s "$SEED" --load-model $MODEL_PATH --overwrite
	else
		ml_train -o "$RUN_DIR" \
				 --eval --onnx --roc --histos --history \
